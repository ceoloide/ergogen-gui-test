      ...
      function (require, module, exports) {
        ...
        ((ConvexHullGrahamScan.prototype = {
          ...
          _findPolarAngle: function (n, t) {
            ...
            return (
              this.reverse ? 0 >= r && (r += 360) : r >= 0 && (r += 360),
              r
            );
          },
      ...
      function (require, module, exports) {
        ...
        (function (MakerJs) {
          ...
          function isPathLine(item) {
            return (
              isPath(item) &&
              item.type == MakerJs.pathType.Line &&
              isPoint(item.end)
            );
          }
          ...
          function isPathCircle(item) {
            return (
              isPath(item) &&
              item.type == MakerJs.pathType.Circle &&
              isNumber(item.radius)
            );
          }
          ...
          function isPathArc(item) {
            return (
              isPath(item) &&
              item.type == MakerJs.pathType.Arc &&
              isNumber(item.radius) &&
              isNumber(item.startAngle) &&
              isNumber(item.endAngle)
            );
          }
          ...
          function isPathArcInBezierCurve(item) {
            return (
              isPathArc(item) &&
              isObject(item.bezierData) &&
              isNumber(item.bezierData.startT) &&
              isNumber(item.bezierData.endT)
            );
          }
          ...
          function isChain(item) {
            var x = item;
            return (
              x && x.links && Array.isArray(x.links) && isNumber(x.pathLength)
            );
          }
          ...
          (function (angle) {
            ...
            function ofArcEnd(arc) {
              ...
              if (arc.endAngle < arc.startAngle) {
                var revolutions = Math.ceil(
                  (arc.startAngle - arc.endAngle) / 360
                );
            ...
            function ofLineInDegrees(line) {
              return noRevolutions(
                toDegrees(ofPointInRadians(line.origin, line.end))
              );
            }
            ...
            function mirror(angleInDegrees, mirrorX, mirrorY) {
              ...
              if (mirrorX) {
                angleInDegrees =
                  (angleInDegrees < 180 ? 180 : 540) - angleInDegrees;
              }
             ...
             */
            var linkLineMap = {};
            linkLineMap[MakerJs.pathType.Arc] = function (
              arc,
              first,
              reversed
            ) {
              var fromEnd = first != reversed;
              var angleToRotate = fromEnd
                ? arc.endAngle - 90
                : arc.startAngle + 90;
              var origin = MakerJs.point.fromArc(arc)[fromEnd ? 1 : 0];
              var end = MakerJs.point.rotate(
                MakerJs.point.add(origin, [arc.radius, 0]),
                angleToRotate,
                origin
              );
              return new MakerJs.paths.Line(
                first ? [end, origin] : [origin, end]
              );
            };
            linkLineMap[MakerJs.pathType.Line] = function (
              line,
              first,
              reversed
            ) {
              return reversed
                ? new MakerJs.paths.Line(line.end, line.origin)
                : line;
            };
            /**
             * @private
            ...
            function ofChainLinkJoint(linkA, linkB) {
              ...
              var result = noRevolutions(
                ofLineInDegrees(linkLines[1]) - ofLineInDegrees(linkLines[0])
              );
          ...
          (function (point) {
            ...
            function closest(referencePoint, pointOptions) {
              ...
              for (var i = 0; i < pointOptions.length; i++) {
                var distance = MakerJs.measure.pointDistance(
                  referencePoint,
                  pointOptions[i]
                );
            ...
            function fromPolar(angleInRadians, radius) {
              return [
                angleInRadians in zero_cos
                  ? 0
                  : MakerJs.round(radius * Math.cos(angleInRadians)),
                angleInRadians in zero_sin
                  ? 0
                  : MakerJs.round(radius * Math.sin(angleInRadians)),
              ];
            }
            ...
            function fromAngleOnCircle(angleInDegrees, circle) {
              return add(
                circle.origin,
                fromPolar(
                  MakerJs.angle.toRadians(angleInDegrees),
                  circle.radius
                )
              );
            }
            ...
            function fromArc(arc) {
              return [
                fromAngleOnCircle(arc.startAngle, arc),
                fromAngleOnCircle(arc.endAngle, arc),
              ];
            }
            ...
            pathEndsMap[MakerJs.pathType.BezierSeed] =
              pathEndsMap[MakerJs.pathType.Line];
            /**
            ...
            function fromSlopeIntersection(lineA, lineB, options) {
              ...
              if (MakerJs.measure.isSlopeParallel(slopeA, slopeB)) {
                if (MakerJs.measure.isSlopeEqual(slopeA, slopeB)) {
                  //check for overlap
                  options.out_AreOverlapped = MakerJs.measure.isLineOverlapping(
                    lineA,
                    lineB,
                    options.excludeTangents
                  );
                }
                return null;
              }
              ...
              if (!slopeA.hasSlope) {
                ...
                var x =
                  (slopeB.yIntercept - slopeA.yIntercept) /
                  (slopeA.slope - slopeB.slope);
            ...
            function midCircle(circle, midAngle) {
              return point.add(
                circle.origin,
                point.fromPolar(
                  MakerJs.angle.toRadians(midAngle),
                  circle.radius
                )
              );
            }
            ...
            function rounded(pointContext, accuracy) {
              return [
                MakerJs.round(pointContext[0], accuracy),
                MakerJs.round(pointContext[1], accuracy),
              ];
            }
            ...
            function rotate(pointToRotate, angleInDegrees, rotationOrigin) {
              ...
              var pointAngleInRadians = MakerJs.angle.ofPointInRadians(
                rotationOrigin,
                pointToRotate
              );
              var d = MakerJs.measure.pointDistance(
                rotationOrigin,
                pointToRotate
              );
              var rotatedPoint = fromPolar(
                pointAngleInRadians + MakerJs.angle.toRadians(angleInDegrees),
                d
              );
        ...
        (function (MakerJs) {
          ...
          (function (path) {
             ...
             */
            var copyPropsMap = {};
            copyPropsMap[MakerJs.pathType.Circle] = function (
              srcCircle,
              destCircle,
              offset
            ) {
            ...
            copyPropsMap[MakerJs.pathType.Arc] = function (
              srcArc,
              destArc,
              offset
            ) {
            ...
            copyPropsMap[MakerJs.pathType.Line] = function (
              srcLine,
              destLine,
              offset
            ) {
            ...
            copyPropsMap[MakerJs.pathType.BezierSeed] = function (
              srcSeed,
              destSeed,
              offset
            ) {
             ...
             */
            var mirrorMap = {};
            mirrorMap[MakerJs.pathType.Line] = function (
              line,
              origin,
              mirrorX,
              mirrorY
              ...
              return new MakerJs.paths.Line(
                origin,
                MakerJs.point.mirror(line.end, mirrorX, mirrorY)
              );
            };
            mirrorMap[MakerJs.pathType.Circle] = function (
              circle,
              origin,
              mirrorX,
              mirrorY
            ...
            mirrorMap[MakerJs.pathType.Arc] = function (
              arc,
              origin,
              mirrorX,
              mirrorY
            ) {
              var startAngle = MakerJs.angle.mirror(
                arc.startAngle,
                mirrorX,
                mirrorY
              );
              var endAngle = MakerJs.angle.mirror(
                MakerJs.angle.ofArcEnd(arc),
                mirrorX,
                mirrorY
              );
              var xor = mirrorX != mirrorY;
              return new MakerJs.paths.Arc(
                origin,
                arc.radius,
            ...
            mirrorMap[MakerJs.pathType.BezierSeed] = function (
              seed,
              origin,
              mirrorX,
              mirrorY
            ) {
              var mirrored = mirrorMap[MakerJs.pathType.Line](
                seed,
                origin,
                mirrorX,
                mirrorY
            ...
            function mirror(pathToMirror, mirrorX, mirrorY) {
              var newPath = null;
              if (pathToMirror) {
                var origin = MakerJs.point.mirror(
                  pathToMirror.origin,
                  mirrorX,
                  mirrorY
                );
             ...
             */
            var moveRelativeMap = {};
            moveRelativeMap[MakerJs.pathType.Line] = function (
              line,
              delta,
              subtract
            ) {
            ...
            moveRelativeMap[MakerJs.pathType.BezierSeed] = function (
              seed,
              delta,
              subtract
            ) {
            ...
            function moveRelative(pathToMove, delta, subtract) {
              if (pathToMove && delta) {
                pathToMove.origin = MakerJs.point.add(
                  pathToMove.origin,
                  delta,
                  subtract
                );
             ...
             */
            var rotateMap = {};
            rotateMap[MakerJs.pathType.Line] = function (
              line,
              angleInDegrees,
              rotationOrigin
            ) {
              line.end = MakerJs.point.rotate(
                line.end,
                angleInDegrees,
                rotationOrigin
              );
            };
            rotateMap[MakerJs.pathType.Arc] = function (
              arc,
              angleInDegrees,
              rotationOrigin
            ) {
              arc.startAngle = MakerJs.angle.noRevolutions(
                arc.startAngle + angleInDegrees
              );
              arc.endAngle = MakerJs.angle.noRevolutions(
                arc.endAngle + angleInDegrees
              );
            };
            rotateMap[MakerJs.pathType.BezierSeed] = function (
              seed,
              angleInDegrees,
              rotationOrigin
            ) {
              rotateMap[MakerJs.pathType.Line](
                seed,
                angleInDegrees,
                rotationOrigin
              );
            ...
            function rotate(pathToRotate, angleInDegrees, rotationOrigin) {
              ...
              pathToRotate.origin = MakerJs.point.rotate(
                pathToRotate.origin,
                angleInDegrees,
                rotationOrigin
              );
            ...
            scaleMap[MakerJs.pathType.BezierSeed] = function (
              seed,
              scaleValue
            ) {
            ...
            function scale(pathToScale, scaleValue) {
              if (!pathToScale || scaleValue === 1 || !scaleValue)
                return pathToScale;
              pathToScale.origin = MakerJs.point.scale(
                pathToScale.origin,
                scaleValue
              );
            ...
            distortMap[MakerJs.pathType.Circle] = function (
              circle,
              scaleX,
              scaleY
            ) {
              var ellipse = new MakerJs.models.Ellipse(
                circle.radius * scaleX,
                circle.radius * scaleY
              );
              ellipse.origin = MakerJs.point.distort(
                circle.origin,
                scaleX,
                scaleY
              );
            ...
            distortMap[MakerJs.pathType.Line] = function (
              line,
              scaleX,
              scaleY
            ) {
              return new MakerJs.paths.Line(
                [line.origin, line.end].map(function (p) {
                  return MakerJs.point.distort(p, scaleX, scaleY);
                })
              );
            };
            distortMap[MakerJs.pathType.BezierSeed] = function (
              seed,
              scaleX,
              scaleY
            ) {
              ...
              if (p) {
                ...
                if (arguments.length === 2) {
                  ...
                  lines.forEach(function (line, i) {
                    useOrigin[i] =
                      MakerJs.point.closest(p, [line.origin, line.end]) ===
                      line.origin;
                  });
                }
             ...
             */
            var alterMap = {};
            alterMap[MakerJs.pathType.Arc] = function (
              arc,
              pathLength,
              distance,
              useOrigin
            ...
            alterMap[MakerJs.pathType.Circle] = function (
              circle,
              pathLength,
              distance,
              useOrigin
            ...
            alterMap[MakerJs.pathType.Line] = function (
              line,
              pathLength,
              distance,
              useOrigin
            ) {
              var delta = MakerJs.point.scale(
                MakerJs.point.subtract(line.end, line.origin),
                distance / pathLength
              );
              if (useOrigin) {
            ...
            numberOfKeyPointsMap[MakerJs.pathType.Circle] = function (
              circle,
              maxPointDistance
            ) {
            ...
            numberOfKeyPointsMap[MakerJs.pathType.Arc] = function (
              arc,
              maxPointDistance
            ) {
              ...
              return Math.max(
                minPoints,
                Math.ceil(len / (maxPointDistance || len))
              );
            };
            /**
            ...
            function zero(pathToZero) {
              ...
              var z = MakerJs.point.subtract(
                pathToZero.origin || [0, 0],
                m.low
              );
              move(pathToZero, z);
              return pathToZero;
        ...
        (function (MakerJs) {
          ...
          (function (path_1) {
            ...
            breakPathFunctionMap[MakerJs.pathType.Arc] = function (
              arc,
              pointOfBreak
            ) {
              var angleAtBreakPoint = MakerJs.angle.ofPointInDegrees(
                arc.origin,
                pointOfBreak
              );
              if (
                MakerJs.measure.isAngleEqual(
                  angleAtBreakPoint,
                  arc.startAngle
                ) ||
                MakerJs.measure.isAngleEqual(angleAtBreakPoint, arc.endAngle)
              ) {
                return null;
              }
              function getAngleStrictlyBetweenArcAngles() {
                ...
                var endAngle =
                  startAngle + MakerJs.angle.ofArcEnd(arc) - arc.startAngle;
                var tries = [0, 1, -1];
                for (var i = 0; i < tries.length; i++) {
                  var add = +360 * tries[i];
                  if (
                    MakerJs.measure.isBetween(
                      angleAtBreakPoint + add,
                      startAngle,
                      endAngle,
                      true
                    )
                    ...
                    return (
                      arc.startAngle + angleAtBreakPoint + add - startAngle
                    );
                  }
                }
                return null;
            ...
            breakPathFunctionMap[MakerJs.pathType.Circle] = function (
              circle,
              pointOfBreak
            ) {
            ...
            breakPathFunctionMap[MakerJs.pathType.Line] = function (
              line,
              pointOfBreak
            ) {
            ...
            var Arc = /** @class */ (function () {
              function Arc() {
                ...
                function getSpan(origin) {
                  var startAngle = MakerJs.angle.ofPointInDegrees(
                    origin,
                    args[clockwise ? 1 : 0]
                  );
                  var endAngle = MakerJs.angle.ofPointInDegrees(
                    origin,
                    args[clockwise ? 0 : 1]
                  );
                ...
                switch (args.length) {
                  case 5:
                    ...
                    var smallestRadius =
                      MakerJs.measure.pointDistance(pointA, pointB) / 2;
                    if (MakerJs.round(this.radius - smallestRadius) <= 0) {
                      ...
                      var intersectionPoints =
                        (_b =
                          (_a = MakerJs.path.intersection(
                            new Circle(pointA, this.radius),
                            new Circle(pointB, this.radius)
                          )) === null || _a === void 0
                            ? void 0
                            : _a.intersectionPoints) !== null && _b !== void 0
                          ? _b
                          : [pointA, pointB];
                      var spans = [];
                  ...
                  case 3:
                    if (MakerJs.isPoint(args[2])) {
                      ...
                      for (var i_1 = 0; i_1 < 3; i_1++) {
                        angles.push(
                          MakerJs.angle.ofPointInDegrees(this.origin, args[i_1])
                        );
                      }
                  ...
                  case 2:
                    ...
                    this.startAngle = MakerJs.angle.ofPointInDegrees(
                      this.origin,
                      args[clockwise ? 1 : 0]
                    );
                    this.endAngle = MakerJs.angle.ofPointInDegrees(
                      this.origin,
                      args[clockwise ? 0 : 1]
                    );
                    break;
                ...
                switch (args.length) {
                  ...
                  case 2:
                    if (MakerJs.isNumber(args[1])) {
                      ...
                      this.radius = MakerJs.measure.pointDistance(
                        this.origin,
                        args[0]
                      );
                    }
                  ...
                  default:
                    ...
                    for (var i = 2; i--; ) {
                      ...
                      perpendiculars.push(
                        MakerJs.path.rotate(lines[i], 90, midpoint)
                      );
                    }
                    //find intersection of slopes of perpendiculars
                    var origin = MakerJs.point.fromSlopeIntersection(
                      perpendiculars[0],
                      perpendiculars[1]
                    );
                    if (origin) {
                      ...
                      throw (
                        'invalid parameters - attempted to construct a circle from 3 points on a line: ' +
                        JSON.stringify(args)
                      );
                    }
                    break;
            ...
            var Parallel = /** @class */ (function () {
              function Parallel(toLine, distance, nearPoint) {
                ...
                function getNewOrigin(offsetAngle) {
                  var origin = MakerJs.point.add(
                    toLine.origin,
                    MakerJs.point.fromPolar(
                      MakerJs.angle.toRadians(angleOfLine + offsetAngle),
                      distance
                    )
                  );
                  return {
                ...
                var newOrigin =
                  newOrigins[0].nearness < newOrigins[1].nearness
                    ? newOrigins[0].origin
                    : newOrigins[1].origin;
        ...
        (function (MakerJs) {
          ...
          (function (model) {
            ...
            function addPath(modelContext, pathContext, pathId, overWrite) {
              ...
              var id = overWrite
                ? pathId
                : getSimilarPathId(modelContext, pathId);
            ...
            function addModel(
              ...
              var id = overWrite
                ? childModelId
                : getSimilarModelId(parentModel, childModelId);
            ...
            function getAllCaptionsOffset(modelContext) {
              ...
              function tryAddCaption(m, offset, layer) {
                if (m.caption) {
                  captions.push({
                    text: m.caption.text,
                    anchor: MakerJs.path.clone(
                      m.caption.anchor,
                      MakerJs.point.add(m.origin, offset)
                    ),
            ...
            function originate(modelToOriginate, origin) {
              ...
              innerOriginate(
                modelToOriginate,
                origin ? MakerJs.point.subtract([0, 0], origin) : [0, 0]
              );
              if (origin) {
            ...
            function mirror(modelToMirror, mirrorX, mirrorY) {
              ...
              if (modelToMirror.origin) {
                newModel.origin = MakerJs.point.mirror(
                  modelToMirror.origin,
                  mirrorX,
                  mirrorY
                );
              }
              ...
              if (modelToMirror.type === MakerJs.models.BezierCurve.typeName) {
                ...
                newModel.seed = MakerJs.path.mirror(
                  modelToMirror.seed,
                  mirrorX,
                  mirrorY
                );
              ...
              if (modelToMirror.paths) {
                ...
                for (var id in modelToMirror.paths) {
                  ...
                  var pathMirrored = MakerJs.path.mirror(
                    pathToMirror,
                    mirrorX,
                    mirrorY
                  );
              ...
              if (modelToMirror.caption) {
                ...
                newModel.caption.anchor = MakerJs.path.mirror(
                  modelToMirror.caption.anchor,
                  mirrorX,
                  mirrorY
                );
              }
            ...
            function moveRelative(modelToMove, delta) {
              if (modelToMove) {
                modelToMove.origin = MakerJs.point.add(
                  modelToMove.origin || MakerJs.point.zero(),
                  delta
                );
              }
              return modelToMove;
            ...
            function prefixPathIds(modelToPrefix, prefix) {
              ...
              for (var i = 0; i < walkedPaths.length; i++) {
                ...
                walkedPath.modelContext.paths[prefix + walkedPath.pathId] =
                  walkedPath.pathContext;
              }
            ...
            function rotate(modelToRotate, angleInDegrees, rotationOrigin) {
              ...
              var offsetOrigin = MakerJs.point.subtract(
                rotationOrigin,
                modelToRotate.origin
              );
            ...
            function scale(modelToScale, scaleValue, scaleOrigin) {
              ...
              if (scaleOrigin && modelToScale.origin) {
                modelToScale.origin = MakerJs.point.scale(
                  modelToScale.origin,
                  scaleValue
                );
              }
            ...
            function addDistortedPath(
              ...
              var distortedPath = MakerJs.path.distort(
                pathToDistort,
                scaleX,
                scaleY
              );
              ...
              if (MakerJs.isPath(distortedPath)) {
                if (distortedPath.type === MakerJs.pathType.BezierSeed) {
                  var curve = new MakerJs.models.BezierCurve(
                    distortedPath,
                    bezierAccuracy
                  );
            ...
            function distort(
              ...
              if (scaleOrigin && modelToDistort.origin) {
                distorted.origin = MakerJs.point.distort(
                  modelToDistort.origin,
                  scaleX,
                  scaleY
                );
              }
              if (modelToDistort.type === MakerJs.models.BezierCurve.typeName) {
                ...
                var bezierPartsByLayer =
                  MakerJs.models.BezierCurve.getBezierSeeds(b, {
                    byLayers: true,
                  });
              ...
              if (modelToDistort.caption) {
                ...
                distorted.caption.anchor = MakerJs.path.distort(
                  modelToDistort.caption.anchor,
                  scaleX,
                  scaleY
                );
              }
            ...
            function convertUnits(modeltoConvert, destUnitType) {
              if (
                modeltoConvert.units &&
                MakerJs.units.isValidUnit(modeltoConvert.units) &&
                MakerJs.units.isValidUnit(destUnitType)
              ) {
                var ratio = MakerJs.units.conversionScale(
                  modeltoConvert.units,
                  destUnitType
                );
                if (ratio != 1) {
              ...
              function walkRecursive(
                ...
                if (modelContext.models) {
                  for (var modelId in modelContext.models) {
                    ...
                    walkRecursive(
                      walkedModel.childModel,
                      walkedModel.layer,
                      newOffset,
                      walkedModel.route,
                      walkedModel.routeKey
                    );
        ...
        (function (MakerJs) {
          ...
          (function (model) {
            ...
            function getNonZeroSegments(pathToSegment, breakPoint) {
              ...
              if (segment2) {
                ...
                for (var i = 2; i--; ) {
                  if (
                    MakerJs.round(
                      MakerJs.measure.pathLength(segments[i]),
                      0.0001
                    ) == 0
                  ) {
                    return null;
            ...
            function breakAlongForeignPath(
              ...
              if (
                MakerJs.measure.isPathEqual(
                  segments[0].absolutePath,
                  foreignPath,
                  0.0001,
                  null,
                  foreignWalkedPath.offset
                )
              ) {
              ...
              var foreignIntersection = MakerJs.path.intersection(
                crossedPath.pathContext,
                foreignPath,
                options
              );
              var intersectionPoints = foreignIntersection
                ? foreignIntersection.intersectionPoints
                : null;
              ...
              for (var i = 0; i < segments.length; i++) {
                var pointsOfInterest = intersectionPoints
                  ? foreignPathEndPoints.concat(intersectionPoints)
                  : foreignPathEndPoints;
                var pointsToCheck = getPointsOnPath(
                  pointsOfInterest,
                  segments[i].absolutePath,
                  popOptions
                );
                ...
                if (pointsToCheck.length > 0) {
                  ...
                  while (!subSegments && p < pointsToCheck.length) {
                    subSegments = getNonZeroSegments(
                      segments[i].absolutePath,
                      pointsToCheck[p]
                    );
                    p++;
            ...
            function isPathInsideModel(
              ...
              var p = MakerJs.point.add(
                MakerJs.point.middle(pathContext),
                pathOffset
              );
              return MakerJs.measure.isPointInsideModel(
                p,
                modelContext,
                options
              );
            ...
            function breakPathsAtIntersections(modelToBreak, modelToIntersect) {
              ...
              if (!modelToIntersect) {
                ...
                modelToIntersectAtlas = new MakerJs.measure.Atlas(
                  modelToIntersect
                );
                modelToIntersectAtlas.measureModels();
              }
              breakAllPathsAtIntersections(
                modelToBreak,
                modelToIntersect || modelToBreak,
                false,
                modelToBreakAtlas,
                modelToIntersectAtlas
              );
              return modelToBreak;
            }
            ...
            function breakAllPathsAtIntersections(
              ...
              var walkModelToBreakOptions = {
                onPath: function (outerWalkedPath) {
                  ...
                  var segment = {
                    absolutePath: MakerJs.path.clone(
                      outerWalkedPath.pathContext,
                      outerWalkedPath.offset
                    ),
                  ...
                  var walkModelToIntersectOptions = {
                    onPath: function (innerWalkedPath) {
                      if (
                        outerWalkedPath.pathContext !==
                          innerWalkedPath.pathContext &&
                        MakerJs.measure.isMeasurementOverlapping(
                          modelToBreakAtlas.pathMap[outerWalkedPath.routeKey],
                          modelToIntersectAtlas.pathMap[
                            innerWalkedPath.routeKey
                          ]
                        )
                      ) {
                        breakAlongForeignPath(
                          thisPath,
                          overlappedSegments,
                          innerWalkedPath
                        );
                    ...
                    beforeChildWalk: function (innerWalkedModel) {
                      ...
                      var innerModelMeasurement =
                        modelToIntersectAtlas.modelMap[
                          innerWalkedModel.routeKey
                        ];
                      return (
                        innerModelMeasurement &&
                        MakerJs.measure.isMeasurementOverlapping(
                          modelToBreakAtlas.pathMap[outerWalkedPath.routeKey],
                          innerModelMeasurement
                        )
                  ...
                  if (checkIsInside) {
                    ...
                    for (var i = 0; i < thisPath.segments.length; i++) {
                      var p = MakerJs.point.middle(
                        thisPath.segments[i].absolutePath
                      );
                      var pointInsideOptions = {
                        measureAtlas: modelToIntersectAtlas,
                        farPoint: farPoint,
                      };
                      thisPath.segments[i].isInside =
                        MakerJs.measure.isPointInsideModel(
                          p,
                          modelToIntersect,
                          pointInsideOptions
                        );
                      thisPath.segments[i].uniqueForeignIntersectionPoints =
                        pointInsideOptions.out_intersectionPoints;
                    }
                  }
              ...
              return {
                ...
                overlappedSegments: overlappedSegments,
              };
            }
            /**
             * @private
             */
            function checkForEqualOverlaps(
              crossedPathsA,
              crossedPathsB,
              ...
              function compareSegments(segment1, segment2) {
                if (
                  MakerJs.measure.isPathEqual(
                    segment1.absolutePath,
                    segment2.absolutePath,
                    pointMatchingDistance
                  )
                ) {
            ...
            function addOrDeleteSegments(
              ...
              function addSegment(modelContext, pathIdBase, segment) {
                ...
                var newRouteKey =
                  id == pathIdBase
                    ...
                    : MakerJs.createRouteKey(
                        crossedPath.route.slice(0, -1).concat([id])
                      );
                segment.addedPath = MakerJs.cloneObject(
                  crossedPath.pathContext
                );
                ...
                if (crossedPath.broken) {
                  ...
                  var measurement = MakerJs.measure.pathExtents(
              ...
              function checkAddSegment(modelContext, pathIdBase, segment) {
                if (
                  (segment.isInside && includeInside) ||
                  (!segment.isInside && includeOutside)
                ) {
                  ...
                  trackDeleted(
                    ...
                    crossedPath.routeKey,
                    'segment is ' +
                      (segment.isInside ? 'inside' : 'outside') +
                      ' intersectionPoints=' +
                      JSON.stringify(segment.uniqueForeignIntersectionPoints)
                  );
                }
              }
              ...
              for (var i = 0; i < crossedPath.segments.length; i++) {
                if (crossedPath.segments[i].duplicate) {
                  if (keepDuplicates) {
                    addSegment(
                      crossedPath.modelContext,
                      crossedPath.pathId,
                      crossedPath.segments[i]
                    );
                  } else {
                    trackDeleted(
                      crossedPath.segments[i].absolutePath,
                      crossedPath.routeKey,
            ...
            function combine(
              ...
              opts.measureA =
                opts.measureA || new MakerJs.measure.Atlas(modelA);
              opts.measureB =
                opts.measureB || new MakerJs.measure.Atlas(modelB);
              //make sure model measurements capture all paths
              opts.measureA.measureModels();
              opts.measureB.measureModels();
              if (!opts.farPoint) {
                var measureBoth = MakerJs.measure.increase(
                  MakerJs.measure.increase(
                    { high: [null, null], low: [null, null] },
                    opts.measureA.modelMap['']
                  ),
                  opts.measureB.modelMap['']
                );
                opts.farPoint = MakerJs.point.add(measureBoth.high, [1, 1]);
              }
              var pathsA = breakAllPathsAtIntersections(
                modelA,
                modelB,
                true,
                opts.measureA,
              ...
              var pathsB = breakAllPathsAtIntersections(
                modelB,
                modelA,
                true,
                opts.measureB,
              ...
              checkForEqualOverlaps(
                pathsA.overlappedSegments,
                pathsB.overlappedSegments,
                opts.pointMatchingDistance
              );
              ...
              for (var i = 0; i < pathsA.crossedPaths.length; i++) {
                addOrDeleteSegments(
                  pathsA.crossedPaths[i],
                  includeAInsideB,
                  includeAOutsideB,
                  true,
                  opts.measureA,
                  function (p, id, reason) {
                ...
                if (!includeAInsideB && !includeBInsideA) {
                  shouldKeep = function (walkedPath) {
                    ...
                    for (var i = 0; i < pathsA.overlappedSegments.length; i++) {
                      if (
                        pathsA.overlappedSegments[i].duplicate &&
                        walkedPath.pathContext ===
                          pathsA.overlappedSegments[i].addedPath
                      ) {
                        return false;
                      }
                    }
        ...
        (function (MakerJs) {
          ...
          var Collector = /** @class */ (function () {
            ...
            Collector.prototype.removeCollection = function (key) {
              var _this = this;
              if (
                this.findCollection(key, function (index) {
            ...
            Collector.prototype.removeItemFromCollection = function (
              key,
              item
            ) {
          ...
          var PointGraph = /** @class */ (function () {
            ...
            PointGraph.prototype.mergePoints = function (withinDistance) {
              ...
              var _loop_2 = function (pointId) {
                ...
                var mergeIds = this_1.kdbush.within(
                  el.point[0],
                  el.point[1],
                  withinDistance
                );
             ...
             * @param withinDistance Distance to consider points equal.
             */
            PointGraph.prototype.mergeNearestSinglePoints = function (
              withinDistance
            ) {
              var _this = this;
              var singles = [];
              ...
              this.kdbush = kdbush(
                singles.map(function (el) {
                  return el.point;
                })
              );
              singles.forEach(function (el) {
                if (el.pointId in _this.merged) return;
                var mergeIds = _this.kdbush.within(
                  el.point[0],
                  el.point[1],
                  withinDistance
                );
                ...
                mergeIds.forEach(function (i) {
                  ...
                  byDistance.push({
                    el: other,
                    distance: MakerJs.measure.pointDistance(
                      other.point,
                      el.point
                    ),
                  });
          ...
          (function (model) {
            ...
            function simplify(modelToSimplify, options) {
              function compareCircles(circleA, circleB) {
                if (
                  Math.abs(circleA.radius - circleB.radius) <=
                  opts.scalarMatchingDistance
                ) {
                  var distance = MakerJs.measure.pointDistance(
                    circleA.origin,
                    circleB.origin
              ...
              var similarLines = new MakerJs.Collector(
                MakerJs.measure.isSlopeEqual
              );
              var map = {};
              ...
              map[MakerJs.pathType.Circle] = function (circleRef) {
                similarCircles.addItemToCollection(
                  circleRef.pathContext,
                  circleRef
                );
              };
              ...
              similarArcs.getCollectionsOfMultiple(function (key, arcRefs) {
                checkForOverlaps(
                  arcRefs,
                  MakerJs.measure.isArcOverlapping,
                  function (arcA, arcB) {
                    //find ends within the other
                    var aEndsInB = MakerJs.measure.isBetweenArcAngles(
                      arcA.endAngle,
                      arcB,
                      false
                    );
                    var bEndsInA = MakerJs.measure.isBetweenArcAngles(
                      arcB.endAngle,
                      arcA,
                      false
                    );
                    ...
                    //save in arcA
                    arcA.startAngle = MakerJs.angle.noRevolutions(
                      ordered[0].startAngle
                    );
              ...
              similarCircles.getCollectionsOfMultiple(
                function (key, circleRefs) {
              ...
              similarLines.getCollectionsOfMultiple(function (slope, arcRefs) {
                checkForOverlaps(
                  arcRefs,
                  MakerJs.measure.isLineOverlapping,
        ...
        (function (MakerJs) {
          ...
          (function (path) {
            ...
            map[MakerJs.pathType.Arc] = function (arc, expansion, isolateCaps) {
              return new MakerJs.models.OvalArc(
                arc.startAngle,
                arc.endAngle,
                arc.radius,
                expansion,
            ...
            map[MakerJs.pathType.Circle] = function (
              circle,
              expansion,
              isolateCaps
            ) {
              return new MakerJs.models.Ring(
                circle.radius + expansion,
                circle.radius - expansion
              );
            };
            map[MakerJs.pathType.Line] = function (
              line,
              expansion,
              isolateCaps
            ) {
              return new MakerJs.models.Slot(
                line.origin,
                line.end,
                expansion,
                isolateCaps
            ...
            function straighten(arc, bevel, prefix, close) {
              ...
              if (arcSpan >= 270) {
                ...
                //30 degrees is the sharpest
                joints = 2;
              }
              var jointAngleInRadians = MakerJs.angle.toRadians(
                arcSpan / joints
              );
              var circumscribedRadius =
                MakerJs.models.Polygon.circumscribedRadius(
                  arc.radius,
                  jointAngleInRadians
                );
              ...
              var a =
                MakerJs.angle.toRadians(arc.startAngle) +
                jointAngleInRadians / 2;
            ...
            function expandPaths(
              ...
              var walkOptions = {
                onPath: function (walkedPath) {
                  ...
                  if (
                    combineOptions.pointMatchingDistance &&
                    MakerJs.measure.pathLength(walkedPath.pathContext) <
                      combineOptions.pointMatchingDistance
                  )
                    return;
                  var expandedPathModel = MakerJs.path.expand(
                    walkedPath.pathContext,
                    distance,
                    true
                  );
                  if (expandedPathModel) {
                    ...
                    var newId = model.getSimilarModelId(
                      result.models['expansions'],
                      walkedPath.pathId
                    );
                    model.prefixPathIds(
                      expandedPathModel,
                    ...
                    result.models['expansions'].models[newId] =
                      expandedPathModel;
                ...
                for (var id in roundCaps.models) {
                  ...
                  model.walk(roundCaps.models[id], {
                    onPath: function (walkedPath) {
                      ...
                      var straightened = MakerJs.path.straighten(
                        arc,
                        joints == 2,
                        walkedPath.pathId + '_',
                        true
                      ...
                      straightCaps.models[id].models[walkedPath.pathId] =
                        straightened;
            ...
            function outline(
              ...
              if (closed) {
                ...
                chains.forEach(function (c) {
                  ...
                  var isInside = MakerJs.measure.isPointInsideModel(
                    MakerJs.point.middle(wp.pathContext),
                    closed,
                    wp.offset
                  );
            ...
            function conversionScale(srcUnitType, destUnitType) {
              ...
              if (!table[srcUnitType][destUnitType]) {
                ...
                addConversion(
                  srcUnitType,
                  destUnitType,
                  table[srcUnitType][base] * table[base][destUnitType]
                );
              }
        ...
        (function (MakerJs) {
          ...
          (function (measure) {
            ...
            function isAngleEqual(angleA, angleB, accuracy) {
              ...
              var d = MakerJs.angle.noRevolutions(
                MakerJs.round(b - a, accuracy)
              );
              return d == 0;
            }
             ...
             */
            var pathAreEqualMap = {};
            pathAreEqualMap[MakerJs.pathType.Line] = function (
              lineA,
              lineB,
              withinPointDistance
            ) {
              return (
                (isPointEqual(
                  ...
                  isPointEqual(lineA.end, lineB.end, withinPointDistance)) ||
                (isPointEqual(lineA.origin, lineB.end, withinPointDistance) &&
                  isPointEqual(lineA.end, lineB.origin, withinPointDistance))
              );
            };
            pathAreEqualMap[MakerJs.pathType.Circle] = function (
              circleA,
              circleB,
              withinPointDistance
            ) {
            ...
            pathAreEqualMap[MakerJs.pathType.Arc] = function (
              arcA,
              arcB,
              withinPointDistance
            ) {
              return (
                pathAreEqualMap[MakerJs.pathType.Circle](
                  arcA,
                  arcB,
                  withinPointDistance
                ) &&
                isAngleEqual(arcA.startAngle, arcB.startAngle) &&
                isAngleEqual(arcA.endAngle, arcB.endAngle)
              );
            };
            /**
              ...
              if (pathA.type == pathB.type) {
                ...
                if (fn) {
                  ...
                  if (pathAOffset || pathBOffset) {
                    MakerJs.path.moveTemporary(
                      [pathA, pathB],
                      [pathAOffset, pathBOffset],
                      getResult
                    );
                  } else {
                    getResult();
            ...
            function isPointEqual(a, b, withinDistance) {
              if (!withinDistance) {
                return (
                  MakerJs.round(a[0] - b[0]) == 0 &&
                  MakerJs.round(a[1] - b[1]) == 0
                );
              } else {
                if (!a || !b) return false;
            ...
            function isPointOnSlope(p, slope, withinDistance) {
              ...
              if (slope.hasSlope) {
                // y = mx * b
                return (
                  Math.abs(p[1] - (slope.slope * p[0] + slope.yIntercept)) <=
                  withinDistance
                );
              } else {
                //vertical slope
            ...
            function isPointOnCircle(p, circle, withinDistance) {
              ...
              var d = Math.abs(
                measure.pointDistance(p, circle.origin) - circle.radius
              );
             ...
             */
            var onPathMap = {};
            onPathMap[MakerJs.pathType.Circle] = function (
              p,
              circle,
              withinDistance
            ) {
            ...
            onPathMap[MakerJs.pathType.Arc] = function (
              p,
              arc,
              withinDistance
            ) {
            ...
            onPathMap[MakerJs.pathType.Line] = function (
              p,
              line,
              withinDistance,
              options
              ...
              var slope =
                (options && options.cachedLineSlope) || measure.lineSlope(line);
              if (options && !options.cachedLineSlope) {
                options.cachedLineSlope = slope;
              }
              return (
                isPointOnSlope(p, slope, withinDistance) &&
                measure.isBetweenPoints(p, line, false)
              );
            };
            /**
            ...
            function isPointOnPath(
              ...
              if (fn) {
                var offsetPath = pathOffset
                  ? MakerJs.path.clone(onPath, pathOffset)
                  : onPath;
            ...
            function isSlopeEqual(slopeA, slopeB) {
              ...
              if (!slopeA.hasSlope && !slopeB.hasSlope) {
                ...
                return (
                  MakerJs.round(
                    slopeA.line.origin[0] - slopeB.line.origin[0]
                  ) == 0
                );
              }
              ...
              return MakerJs.round(averageYs[0] - averageYs[1], 0.00001) == 0;
            }
            ...
            function isSlopeParallel(slopeA, slopeB) {
              ...
              if (
                slopeA.hasSlope &&
                slopeB.hasSlope &&
                MakerJs.round(slopeA.slope - slopeB.slope, 0.00001) == 0
              ) {
                //lines are parallel
                return true;
              }
        ...
        (function (MakerJs) {
          ...
          (function (measure) {
            ...
            function isArcConcaveTowardsPoint(arc, towardsPoint) {
              ...
              var midPointToNearPoint = new MakerJs.paths.Line(
                MakerJs.point.middle(arc),
                towardsPoint
              );
              var options = {};
              var intersectionPoint = MakerJs.path.intersection(
                midPointToNearPoint,
                new MakerJs.paths.Chord(arc),
                options
              );
            ...
            function isArcSpanOverlapping(arcA, arcB, excludeTangents) {
              ...
              return (
                checkAngles(arcA, arcB) ||
                checkAngles(arcB, arcA) ||
                (arcA.startAngle == arcB.startAngle &&
                  arcA.endAngle == arcB.endAngle)
              );
            }
            ...
            function isBetween(valueInQuestion, limitA, limitB, exclusive) {
              if (exclusive) {
                return (
                  Math.min(limitA, limitB) < valueInQuestion &&
                  valueInQuestion < Math.max(limitA, limitB)
                );
              } else {
                return (
                  Math.min(limitA, limitB) <= valueInQuestion &&
            ...
            function isBetweenArcAngles(angleInQuestion, arc, exclusive) {
              ...
              return (
                isBetween(angleInQuestion, startAngle, endAngle, exclusive) ||
                isBetween(
                  angleInQuestion,
                  startAngle + 360,
                  endAngle + 360,
            ...
            function isBetweenPoints(pointInQuestion, line, exclusive) {
              ...
              for (var i = 2; i--; ) {
                if (
                  MakerJs.round(line.origin[i] - line.end[i], 0.000001) == 0
                ) {
                ...
                if (
                  !isBetween(
                    MakerJs.round(pointInQuestion[i]),
                    origin_value,
                    end_value,
                    exclusive
                  )
            ...
            function isLineOverlapping(lineA, lineB, excludeTangents) {
              ...
              return (
                checkPoints(0, lineA, lineB) || checkPoints(1, lineB, lineA)
              );
            }
            ...
            function isMeasurementOverlapping(measureA, measureB) {
              for (var i = 2; i--; ) {
                if (
                  !(
                    MakerJs.round(measureA.low[i] - measureB.high[i]) <= 0 &&
                    MakerJs.round(measureA.high[i] - measureB.low[i]) >= 0
                  )
                )
                  return false;
              }
            ...
            function lineSlope(line) {
              ...
              if (MakerJs.round(dx, 0.000001) == 0) {
                return {
                  line: line,
                  hasSlope: false,
                };
            ...
            pathExtentsMap[MakerJs.pathType.Arc] = function (arc) {
              ...
              function extremeAngle(xyAngle, value, fn) {
                var extremePoint = getExtremePoint(
                  arcPoints[0],
                  arcPoints[1],
                  fn
                );
            ...
            function modelExtents(modelToMeasure, atlas) {
              function increaseParentModel(childRoute, childMeasurement) {
                ...
                if (!(parentRouteKey in atlas.modelMap)) {
                  ...
                  atlas.modelMap[parentRouteKey] =
                    cloneMeasure(childMeasurement);
                } else {
              ...
              var walkOptions = {
                onPath: function (walkedPath) {
                  ...
                  if (!(walkedPath.routeKey in atlas.pathMap)) {
                    atlas.pathMap[walkedPath.routeKey] = pathExtents(
                      walkedPath.pathContext,
                      walkedPath.offset
                    );
                  }
                  increaseParentModel(
                    walkedPath.route,
                    atlas.pathMap[walkedPath.routeKey]
                  );
                },
                afterChildWalk: function (walkedModel) {
                  ...
                  increaseParentModel(
                    walkedModel.route,
                    atlas.modelMap[walkedModel.routeKey]
                  );
                },
              };
            ...
            function isCircular(bounds) {
              for (var i = 1; i < 3; i++) {
                if (
                  ...
                  !(MakerJs.round(bounds[0].width - bounds[i].width) === 0)
                ) {
                  return false;
                }
              }
            ...
            function getAngledBounds(
              ...
              var result = {
                ...
                middle: new MakerJs.paths.Line(
                  [m.low[0], m.center[1]],
                  [m.high[0], m.center[1]]
                ),
                top: new MakerJs.paths.Line(m.high, [m.low[0], m.high[1]]),
              };
              [result.top, result.middle, result.bottom].forEach(
                function (line) {
            ...
            function hexSolution(lines, bounds) {
              ...
              var radiusViaSide =
                MakerJs.solvers.equilateralSide(altitudeViaSide);
              //find peaks, then find highest peak
              var peakPoints = [
                MakerJs.point.fromSlopeIntersection(lines[1], lines[2]),
                MakerJs.point.fromSlopeIntersection(lines[4], lines[5]),
              ];
              ...
              if (radiusViaPeak > radiusViaSide) {
                var altitudeViaPeak =
                  MakerJs.solvers.equilateralAltitude(radiusViaPeak);
                ...
                return {
                  origin: pointOnSlopeAtX(
                    bounds[2 - peakNum].middle,
                    peakX + minHalfGap
                  ),
            ...
            function boundingHexagon(modelToMeasure) {
              ...
              if (isCircular(bounds)) {
                return result(
                  MakerJs.solvers.equilateralSide(bounds[0].width / 2),
                  bounds[0].center,
                  'solved as circular'
                );
              }
              var perimeters = bounds
                .map(function (b) {
                  return b.top;
                })
                .concat(
              ...
              return result(
                solution.radius,
                p,
                'solved by ' + solution.index + ' as ' + solution.type
              );
            }
            ...
            function addUniquePoints(pointArray, pointsToAdd) {
              ...
              pointsToAdd.forEach(function (p) {
                if (!measure.isPointDistinct(p, pointArray, 0.00000001)) return;
                pointArray.push(p);
                added++;
              });
              return added;
            ...
            function isPointInsideModel(pointToCheck, modelContext, options) {
              ...
              if (!options.farPoint) {
                options.farPoint = getFarPoint(
                  modelContext,
                  options.farPoint,
                  options.measureAtlas
                );
              }
              options.out_intersectionPoints = [];
              var isInside;
              var lineToFarPoint = new MakerJs.paths.Line(
                pointToCheck,
                options.farPoint
              );
              ...
              var walkOptions = {
                onPath: function (walkedPath) {
                  if (
                    ...
                    !isMeasurementOverlapping(
                      measureFarPoint,
                      options.measureAtlas.pathMap[walkedPath.routeKey]
                    )
                  ) {
                    return;
                  }
                  var intersectOptions = { path2Offset: walkedPath.offset };
                  var farInt = MakerJs.path.intersection(
                    lineToFarPoint,
                    walkedPath.pathContext,
                    intersectOptions
                  );
                  if (farInt) {
                    var added = addUniquePoints(
                      options.out_intersectionPoints,
                      farInt.intersectionPoints
                    );
                ...
                beforeChildWalk: function (innerWalkedModel) {
                  ...
                  //see if there is a model measurement. if not, it is because the model does not contain paths.
                  var innerModelMeasurement =
                    options.measureAtlas.modelMap[innerWalkedModel.routeKey];
                  return (
                    innerModelMeasurement &&
                    isMeasurementOverlapping(
                      measureFarPoint,
          ...
          (function (exporter) {
            ...
            function toDXF(itemToExport, options) {
              ...
              if (MakerJs.isModel(itemToExport)) {
                ...
                if (modelToExport.exporterOptions) {
                  MakerJs.extendObject(
                    opts,
                    modelToExport.exporterOptions['toDXF']
                  );
                }
              }
              ...
              function defaultLayer(pathContext, parentLayer) {
                var layerId =
                  (pathContext && pathContext.layer) || parentLayer || '0';
              ...
              map[MakerJs.pathType.Line] = function (line, offset, layer) {
                var lineEntity = {
                  ...
                  vertices: [
                    {
                      x: MakerJs.round(
                        line.origin[0] + offset[0],
                        opts.accuracy
                      ),
                      y: MakerJs.round(
                        line.origin[1] + offset[1],
                        opts.accuracy
                      ),
                    },
                    {
              ...
              map[MakerJs.pathType.Circle] = function (circle, offset, layer) {
                var circleEntity = {
                  ...
                  center: {
                    x: MakerJs.round(
                      circle.origin[0] + offset[0],
                      opts.accuracy
                    ),
                    y: MakerJs.round(
                      circle.origin[1] + offset[1],
                      opts.accuracy
                    ),
                  },
              ...
              function polyline(c) {
                ...
                c.chain.links.forEach(function (link, i) {
                  var bulge;
                  if (
                    link.walkedPath.pathContext.type === MakerJs.pathType.Arc
                  ) {
                    var arc = link.walkedPath.pathContext;
                    bulge = MakerJs.round(
                      Math.tan(
                        MakerJs.angle.toRadians(MakerJs.angle.ofArcSpan(arc)) /
                          4
                      ),
                      opts.accuracy
                    );
              ...
              function text(caption) {
                ...
                var textEntity = {
                  ...
                  layer: layerId,
                  textHeight:
                    (layerOptions && layerOptions.fontSize) || opts.fontSize,
                  text: caption.text,
                  halign: 4,
                  valign: 0,
                  rotation: MakerJs.angle.ofPointInDegrees(
                    caption.anchor.origin,
                    caption.anchor.end
                  ),
                };
                return textEntity;
              ...
              function layersOut() {
                ...
                layerIds.forEach(function (layerId) {
                  var layerOptions = colorLayerOptions(layerId);
                  if (layerOptions) {
                    layerTable.layers[layerId] = layerOut(
                      layerId,
                      layerOptions.color
                    );
                  }
                ...
                var cb = function (chains, loose, layer) {
                  chains.forEach(function (c) {
                    if (
                      c.endless &&
                      c.links.length === 1 &&
                      c.links[0].walkedPath.pathContext.type ===
              ...
              entities(
                walkedPaths,
                chainsOnLayers,
                MakerJs.model.getAllCaptionsOffset(modelToExport)
              );
              header();
              lineTypesOut();
              layersOut();
        ...
        (function (MakerJs) {
          ...
          (function (solvers) {
            ...
            function solveTriangleSSS(lengthA, lengthB, lengthC) {
              return MakerJs.angle.toDegrees(
                Math.acos(
                  (lengthB * lengthB + lengthC * lengthC - lengthA * lengthA) /
                    (2 * lengthB * lengthC)
                )
              );
            }
            ...
            function solveTriangleASA(
              oppositeAngleInDegrees,
              lengthOfSideBetweenAngles,
              otherAngleInDegrees
            ) {
              var angleOppositeSide =
                180 - oppositeAngleInDegrees - otherAngleInDegrees;
              return (
                (lengthOfSideBetweenAngles *
                  Math.sin(MakerJs.angle.toRadians(oppositeAngleInDegrees))) /
                Math.sin(MakerJs.angle.toRadians(angleOppositeSide))
              );
            }
            ...
            function circleTangentAngles(a, b, inner) {
              ...
              if (!inner && MakerJs.round(a.radius - b.radius) == 0) {
                ...
                var diff = new MakerJs.paths.Circle(
                  a.radius > b.radius ? [0, 0] : [distance, 0],
                  inner ? a.radius + b.radius : Math.abs(a.radius - b.radius)
                );
        ...
        (function (MakerJs) {
          ...
          (function (path) {
            ...
            map[MakerJs.pathType.Arc][MakerJs.pathType.Arc] = function (
              arc1,
              arc2,
              options,
              swapOffsets
                    ...
                    if (arc1Angles.length === 1 || arc2Angles.length === 1) {
                      for (var i1 = 0; i1 < arc1Angles.length; i1++) {
                        for (var i2 = 0; i2 < arc2Angles.length; i2++) {
                          var p1 = MakerJs.point.fromAngleOnCircle(
                            arc1Angles[i1],
                            arc1
                          );
                          var p2 = MakerJs.point.fromAngleOnCircle(
                            arc2Angles[i2],
                            arc2
                          );
                          //if they do not correspond then they don't intersect
                          if (MakerJs.measure.isPointEqual(p1, p2, 0.0001)) {
                            result = {
                      ...
                      result = {
                        intersectionPoints: pointsFromAnglesOnCircle(
                          arc1Angles,
                          arc1
                        ),
            ...
            map[MakerJs.pathType.Arc][MakerJs.pathType.Circle] = function (
              arc,
              circle,
              options,
              swapOffsets
                ...
                if (angles) {
                  ...
                  if (arcAngles) {
                    ...
                    if (arcAngles.length == 2) {
                      ...
                      var index = findCorrespondingAngleIndex(
                        angles[0],
                        arcAngles[0]
                      );
            ...
            map[MakerJs.pathType.Arc][MakerJs.pathType.Line] = function (
              arc,
              line,
              options,
              swapOffsets
            ...
            map[MakerJs.pathType.Circle][MakerJs.pathType.Arc] = function (
              circle,
              arc,
              options
            ) {
              var result = map[MakerJs.pathType.Arc][MakerJs.pathType.Circle](
                arc,
                circle,
                options,
                true
            ...
            map[MakerJs.pathType.Circle][MakerJs.pathType.Circle] = function (
              circle1,
              circle2,
              options,
              swapOffsets
            ...
            map[MakerJs.pathType.Circle][MakerJs.pathType.Line] = function (
              circle,
              line,
              options,
              swapOffsets
            ...
            map[MakerJs.pathType.Line][MakerJs.pathType.Arc] = function (
              line,
              arc,
              options
            ) {
              var result = map[MakerJs.pathType.Arc][MakerJs.pathType.Line](
                arc,
                line,
                options,
                true
            ...
            map[MakerJs.pathType.Line][MakerJs.pathType.Circle] = function (
              line,
              circle,
              options
            ) {
              var result = map[MakerJs.pathType.Circle][MakerJs.pathType.Line](
                circle,
                line,
                options,
                true
            ...
            map[MakerJs.pathType.Line][MakerJs.pathType.Line] = function (
              line1,
              line2,
              options,
              swapOffsets
              ...
              moveTemp([line1, line2], options, swapOffsets, function () {
                var intersectionPoint = MakerJs.point.fromSlopeIntersection(
                  line1,
                  line2,
                  options
                );
                if (intersectionPoint) {
                  ...
                  if (
                    MakerJs.measure.isBetweenPoints(
                      intersectionPoint,
                      line1,
                      options.excludeTangents
                    ) &&
            ...
            function moveTemp(pathsToOffset, options, swapOffsets, task) {
              var offsets = swapOffsets
                ? [options.path2Offset, options.path1Offset]
                : [options.path1Offset, options.path2Offset];
            ...
            function pointsFromAnglesOnCircle(anglesInDegrees, circle) {
              ...
              for (var i = 0; i < anglesInDegrees.length; i++) {
                result.push(
                  MakerJs.point.fromAngleOnCircle(anglesInDegrees[i], circle)
                );
              }
              return result;
            }
            ...
            function lineToCircle(line, circle, options) {
              ...
              //clone the line
              var clonedLine = new MakerJs.paths.Line(
                MakerJs.point.subtract(line.origin, circle.origin),
                MakerJs.point.subtract(line.end, circle.origin)
              );
              //get angle of line
              ...
              if (lineYabs == radius) {
                ...
                function intersectionBetweenEndpoints(x, angleOfX) {
                  if (
                    MakerJs.measure.isBetween(
                      MakerJs.round(x),
                      MakerJs.round(clonedLine.origin[0]),
                      MakerJs.round(clonedLine.end[0]),
                      options.excludeTangents
                    )
                  ) {
                ...
                var intersectDegrees =
                  MakerJs.angle.toDegrees(intersectRadians);
                //line may intersect in 2 places
                var intersectX = Math.cos(intersectRadians) * radius;
                intersectionBetweenEndpoints(
                  -intersectX,
                  180 - intersectDegrees
                );
            ...
            function circleToCircle(circle1, circle2, options) {
              ...
              var offset = MakerJs.point.subtract(
                MakerJs.point.zero(),
                circle1.origin
              );
              //clone circle1 and move to origin
              var c1 = new MakerJs.paths.Circle(
                MakerJs.point.zero(),
                circle1.radius
              );
              //clone circle2 and move relative to circle1
              var c2 = new MakerJs.paths.Circle(
                MakerJs.point.subtract(circle2.origin, circle1.origin),
                circle2.radius
              );
              //rotate circle2 to horizontal, c2 will be to the right of the origin.
              var c2Angle = MakerJs.angle.ofPointInDegrees(
                MakerJs.point.zero(),
                c2.origin
              );
              ...
              function bothAngles(oneAngle) {
                return [
                  unRotate(oneAngle),
                  unRotate(MakerJs.angle.mirror(oneAngle, false, true)),
                ];
              }
              var c1IntersectionAngle = MakerJs.solvers.solveTriangleSSS(
                c2.radius,
                c1.radius,
                x
              );
              var c2IntersectionAngle = MakerJs.solvers.solveTriangleSSS(
                c1.radius,
                x,
                c2.radius
              );
          ...
          (function (path) {
            ...
            function getMatchingPointProperties(pathA, pathB, options) {
              ...
              function check(iA, iB) {
                if (
                  MakerJs.measure.isPointEqual(
                    pathAProperties[iA].point,
                    pathBProperties[iB].point,
                    0.0001
                  )
                ) {
                  result = [
             ...
             * @private
             */
            function populateShardPointsFromReferenceCircle(
              filletRadius,
              center,
              properties,
              options
            ) {
              var referenceCircle = new MakerJs.paths.Circle(
                center,
                filletRadius
              );
              //get reference circle intersection points
              for (var i = 0; i < 2; i++) {
                var circleIntersection = path.intersection(
                  referenceCircle,
                  properties[i].path
                );
                ...
                properties[i].shardPoint =
                  circleIntersection.intersectionPoints[0];
                if (
                  MakerJs.measure.isPointEqual(
                    properties[i].point,
                    circleIntersection.intersectionPoints[0],
                  ...
                  if (circleIntersection.intersectionPoints.length > 1) {
                    properties[i].shardPoint =
                      circleIntersection.intersectionPoints[1];
                  } else {
                    return false;
                  }
                }
             ...
             */
            var guidePathMap = {};
            guidePathMap[MakerJs.pathType.Arc] = function (
              arc,
              filletRadius,
              nearPoint,
              shardPoint,
              ...
              var guideArcShard = cloneAndBreakPath(arc, shardPoint)[
                isStart ? 0 : 1
              ];
              if (guideArcShard) {
                if (
                  MakerJs.measure.isArcConcaveTowardsPoint(
                    guideArcShard,
                    nearPoint
                  )
                ) {
            ...
            guidePathMap[MakerJs.pathType.Line] = function (
              line,
              filletRadius,
              nearPoint,
              shardPoint,
             ...
             */
            var filletResultMap = {};
            filletResultMap[MakerJs.pathType.Arc] = function (
              arc,
              propertyName,
              filletRadius,
              filletCenter
              ...
              if (
                !MakerJs.measure.isArcConcaveTowardsPoint(arc, filletCenter)
              ) {
                filletAngle += 180;
              }
              return {
            ...
            filletResultMap[MakerJs.pathType.Line] = function (
              line,
              propertyName,
              filletRadius,
              filletCenter
              ...
              if (intersectionPoint) {
                return {
                  filletAngle: MakerJs.angle.ofPointInDegrees(
                    filletCenter,
                    intersectionPoint
                  ),
                  clipPath: function () {
            ...
            function dogbone(lineA, lineB, filletRadius, options) {
              ...
              if (
                MakerJs.isPathLine(lineA) &&
                MakerJs.isPathLine(lineB) &&
                filletRadius &&
                filletRadius > 0
              ) {
                var opts = {
                ...
                var commonProperty = getMatchingPointProperties(
                  lineA,
                  lineB,
                  options
                );
                if (commonProperty) {
                  ...
                  var span = new MakerJs.paths.Line(
                    commonProperty[0].oppositePoint,
                    commonProperty[1].oppositePoint
                  );
                  var midRatioPoint = MakerJs.point.middle(span, ratio);
                  //use the bisection theorem to get the angle bisecting the lines
                  var bisectionAngle = MakerJs.angle.ofPointInDegrees(
                    commonProperty[0].point,
                    midRatioPoint
                  );
                  var center = MakerJs.point.add(
                    commonProperty[0].point,
                    MakerJs.point.fromPolar(
                      MakerJs.angle.toRadians(bisectionAngle),
                      filletRadius
                    )
                  );
                  if (
                  ...
                  var filletArc = new MakerJs.paths.Arc(
                    center,
                    filletRadius,
                    results[0].filletAngle,
                    results[1].filletAngle
                  );
                  //make sure midpoint of fillet is outside of the angle
                  if (
                    MakerJs.round(
                      MakerJs.angle.noRevolutions(
                        MakerJs.angle.ofArcMiddle(filletArc)
                      )
                    ) == MakerJs.round(bisectionAngle)
                  ) {
            ...
            function fillet(pathA, pathB, filletRadius, options) {
              if (pathA && pathB && filletRadius && filletRadius > 0) {
                ...
                if (commonProperty) {
                  ...
                  var intersectionPoint = path.intersection(
                    guidePaths[0],
                    guidePaths[1]
                  );
                  if (intersectionPoint) {
                    ...
                    if (
                      MakerJs.round(
                        results[0].filletAngle - results[1].filletAngle
                      ) == 0
                    )
                      return null;
                    var filletArc = new MakerJs.paths.Arc(
        ...
        (function (MakerJs) {
          ...
          (function (chain) {
            ...
            function chainFillet(traditional, chainToFillet, filletSpec) {
              ...
              function add(i1, i2) {
                var p1 = links[i1].walkedPath,
                  p2 = links[i2].walkedPath;
                if (
                  p1.modelContext === p2.modelContext &&
                  p1.modelContext.type == MakerJs.models.BezierCurve.typeName
                )
                  return;
                MakerJs.path.moveTemporary(
                  [p1.pathContext, p2.pathContext],
                  [p1.offset, p2.offset],
                  function () {
                    var filletRadius;
                    if (MakerJs.isObject(filletSpec)) {
                      var a = MakerJs.angle.ofChainLinkJoint(
                        links[i1],
                        links[i2]
                      );
                    ...
                    var filletArc;
                    if (traditional) {
                      filletArc = MakerJs.path.fillet(
                        p1.pathContext,
                        p2.pathContext,
                        filletRadius
                      );
                    } else {
                      filletArc = MakerJs.path.dogbone(
                        p1.pathContext,
                        p2.pathContext,
                        filletRadius
                      );
          ...
          (function (model) {
            ...
            function followLinks(pointGraph, chainFound, chainNotFound) {
              function followLink(currLink, chain, firstLink) {
                while (currLink) {
                  ...
                  if (
                    nextLink.walkedPath.pathContext ===
                    firstLink.walkedPath.pathContext
                  ) {
              ...
              pointGraph.forEachPoint(function (p, values, pointId, el) {
                if (el.valueIds.length > 0) {
                  ...
                  if (chain.endless) {
                    ...
                    chain.pathLength -=
                      chain.links[chain.links.length - 1].pathLength;
            ...
            function findChains(modelContext) {
              ...
              var walkOptions = {
                onPath: function (walkedPath) {
                  ...
                  var pathLength = MakerJs.measure.pathLength(
                    walkedPath.pathContext
                  );
                  //circles are loops by nature
                  if (
                    walkedPath.pathContext.type === MakerJs.pathType.Circle ||
                    (walkedPath.pathContext.type === MakerJs.pathType.Arc &&
                      MakerJs.round(
                        MakerJs.angle.ofArcSpan(walkedPath.pathContext) - 360
                      ) === 0) ||
                    (walkedPath.pathContext.type ===
                      MakerJs.pathType.BezierSeed &&
                      MakerJs.measure.isPointEqual(
                        walkedPath.pathContext.origin,
                        walkedPath.pathContext.end,
                        opts.pointMatchingDistance
                      ))
                  ) {
                    var chain = {
                    ...
                    var endPoints = MakerJs.point.fromPathEnds(
                      walkedPath.pathContext,
                      walkedPath.offset
                    );
                    for (var i = 0; i < 2; i++) {
              ...
              var _loop_3 = function (layer_2) {
                ...
                followLinks(
                  pointGraph,
                  function (chain, checkEndless) {
                    if (checkEndless) {
                      chain.endless = MakerJs.measure.isPointEqual(
                        linkEndpoint(chain.links[0], true),
                        linkEndpoint(
                          chain.links[chain.links.length - 1],
                          false
                        ),
                        opts.pointMatchingDistance
                      );
                ...
                if (opts.contain) {
                  containChainsOptions = MakerJs.isObject(opts.contain)
                    ? opts.contain
                    : { alternateDirection: false };
                  containedChains = getContainment(
                    chainsByLayer[layer_2],
                    containChainsOptions
                  );
              ...
              if (opts.alternateDirection) {
                function alternate(chains, shouldBeClockwise) {
                  chains.forEach(function (chainContext, i) {
                    var isClockwise =
                      MakerJs.measure.isChainClockwise(chainContext);
                    if (isClockwise !== null) {
                      if (
                        (!isClockwise && shouldBeClockwise) ||
                        (isClockwise && !shouldBeClockwise)
                      ) {
            ...
            function getBezierModels(modelContext) {
              ...
              function checkIsBezier(wm) {
                if (
                  wm.childModel.type === MakerJs.models.BezierCurve.typeName
                ) {
                  beziers.push(wm);
                }
              }
          ...
          (function (chain) {
            ...
            function toNewModel(chainContext, detachFromOldModel) {
              ...
              for (var i = 0; i < chainContext.links.length; i++) {
                ...
                if (wp.pathContext.type === MakerJs.pathType.BezierSeed) {
                  ...
                  var modelId = MakerJs.model.getSimilarModelId(
                    result,
                    wp.pathId
                  );
                  result.models[modelId] = MakerJs.model.moveRelative(
                    new MakerJs.models.BezierCurve(wp.pathContext),
                    wp.offset
                  );
                } else {
                  var newPath;
                  ...
                  var pathId = MakerJs.model.getSimilarPathId(
                    result,
                    wp.pathId
                  );
                  result.paths[pathId] = MakerJs.path.moveRelative(
                    newPath,
                    wp.offset
                  );
                }
            ...
            function toPoints(chainContext, distanceOrDistances, maxPoints) {
              ...
              for (var i = 0; i < chainContext.links.length; i++) {
                ...
                while (MakerJs.round(len - t) > 0) {
                  ...
                  result.push(
                    MakerJs.point.add(
                      MakerJs.point.middle(wp.pathContext, r),
                      wp.offset
                    )
                  );
            ...
            function toKeyPoints(chainContext, maxArcFacet) {
              ...
              for (var i = 0; i < chainContext.links.length; i++) {
                var link = chainContext.links[i];
                var wp = link.walkedPath;
                var keyPoints = MakerJs.path.toKeyPoints(
                  wp.pathContext,
                  maxArcFacet
                );
        ...
        (function (MakerJs) {
          ...
          (function (model) {
            ...
            var DeadEndFinder = /** @class */ (function () {
              ...
              DeadEndFinder.prototype.load = function () {
                ...
                var walkOptions = {
                  onPath: function (walkedPath) {
                    ...
                    for (var i = 2; i--; ) {
                      _this.pointMap.insertValueIdAtPoint(
                        valueId,
                        endPoints[i]
                      );
                    }
              ...
              DeadEndFinder.prototype.removePath = function (
                el,
                valueId,
                current
              ) {
                var value = this.pointMap.values[valueId];
                var otherPointId = this.getOtherPointId(
                  value.endPoints,
                  el.pointId
                );
              ...
              DeadEndFinder.prototype.getOtherPointId = function (
                endPoints,
                pointId
              ) {
            ...
            function removeDeadEnds(
              ...
              var options = {
                pointMatchingDistance: pointMatchingDistance || 0.005,
                keep: keep,
              };
          ...
          (function (exporter) {
            ...
            function chainToJscadScript(chainContext, facetSize, accuracy) {
              ...
              beginMap[MakerJs.pathType.Circle] = function (circle, link) {
                var circleOptions = {
                  center: MakerJs.point.rounded(
                    MakerJs.point.add(circle.origin, link.walkedPath.offset),
                    accuracy
                  ),
              ...
              beginMap[MakerJs.pathType.Arc] = function (arc, link) {
                ...
                var arcOptions = {
                  center: MakerJs.point.rounded(
                    MakerJs.point.add(arc.origin, link.walkedPath.offset),
                    accuracy
                  ),
              ...
              appendMap[MakerJs.pathType.Line] = function (line, link) {
                ...
                var endPoint = MakerJs.point.rounded(
                  link.endPoints[reverse ? 0 : 1],
                  accuracy
                );
              ...
              appendMap[MakerJs.pathType.Arc] = function (arc, link) {
                ...
                append(
                  wrap(
                    '.appendArc',
                    JSON.stringify(endPoint) + ',' + JSON.stringify(arcOptions),
                    true
                  )
                );
              };
              ...
              for (var i = 0; i < chainContext.links.length; i++) {
                ...
                var fn = first
                  ? beginMap[pathContext.type]
                  : appendMap[pathContext.type];
                if (fn) {
                  fn(pathContext, link);
                }
                if (exit) {
            ...
            function makePhasedCallback(originalCb, phaseStart, phaseSpan) {
              return function statusCallback(status) {
                originalCb &&
                  originalCb({
                    progress: phaseStart + (status.progress * phaseSpan) / 100,
                  });
              };
            }
            /**
            ...
            function convertChainsTo2D(
              ...
              function unionize(phaseStart, phaseSpan, arr) {
                ...
                jsCadCagOptions.statusCallback &&
                  jsCadCagOptions.statusCallback({
                    progress:
                      phaseStart + (phaseSpan * status.complete) / status.total,
                  });
                return result;
              }
              ...
              jsCadCagOptions.statusCallback &&
                jsCadCagOptions.statusCallback({ progress: 25 });
              var chainsResult = MakerJs.model.findChains(
                modelToExport,
                options
              );
              ...
              jsCadCagOptions.statusCallback &&
                jsCadCagOptions.statusCallback({ progress: 50 });
              var closedCount = 0;
              ...
              if (closedCount === 0) {
                jsCadCagOptions.statusCallback &&
                  jsCadCagOptions.statusCallback({ progress: 100 });
            ...
            function convert2Dto3D(
              ...
              options.byLayers =
                options.byLayers || (options.layerOptions && true);
              ...
              if (options.byLayers) {
                for (var layerId in result2D) {
                  ...
                  var csg = to3D(
                    result2D[layerId],
                    layerOptions.extrude || options.extrude,
                    getDefinedNumber(layerOptions.z, options.z)
                  );
                  csgs.push(csg);
                }
              } else {
              ...
              csgs.forEach(function (el, i) {
                ...
                options.statusCallback({
                  progress: status.complete / status.total,
                });
              });
              return result;
            }
            ...
            function toJscadScript(modelToExport, options) {
              ...
              function _chainToJscadScript(c, maxArcFacet) {
                return wrap(
                  chainToJscadScript(c, maxArcFacet, options.accuracy)
                );
              }
              ...
              function to3D(cag, extrude, z) {
                var csg =
                  cag + '.extrude({ offset: [0, 0, '.concat(extrude, '] })');
                if (z) {
              ...
              return 'function '
                .concat(options.functionName || 'main', '(){')
                .concat(wrap('return '.concat(result, ';')), '}')
                .concat(nl);
            }
        ...
        (function (MakerJs) {
          ...
          (function (exporter) {
            ...
            function toPDF(doc, modelToExport, options) {
              ...
              if (exportUnits) {
                //convert to inch
                scale = MakerJs.units.conversionScale(
                  exportUnits,
              ...
              var scaledModel = MakerJs.model.scale(
                MakerJs.cloneObject(modelToExport),
                scale
              );
              ...
              MakerJs.model.findChains(
                ...
                function (chains, loose, layer) {
                  function single(walkedPath) {
                    var pathData = exporter.pathToSVGPathData(
                      walkedPath.pathContext,
                      walkedPath.offset,
                      offset
                    );
              ...
              MakerJs.model
                .getAllCaptionsOffset(scaledModel)
                .forEach(function (caption) {
                  ...
                  var textCenter = [
                    doc.widthOfString(text) / 2,
                    doc.heightOfString(text) / 2,
                  ];
        ...
        (function (MakerJs) {
          ...
          (function (exporter) {
            ...
            chainLinkToPathDataMap[MakerJs.pathType.Arc] = function (
              arc,
              endPoint,
              reversed,
              d,
              ...
              svgArcData(
                ...
                endPoint,
                accuracy,
                MakerJs.angle.ofArcSpan(arc) > 180,
                reversed
                  ? arc.startAngle > arc.endAngle
                  : arc.startAngle < arc.endAngle
              );
            };
            chainLinkToPathDataMap[MakerJs.pathType.Line] = function (
              line,
              endPoint,
              reversed,
              d,
              ...
              d.push(
                'L',
                MakerJs.round(endPoint[0], accuracy),
                MakerJs.round(endPoint[1], accuracy)
              );
            };
            chainLinkToPathDataMap[MakerJs.pathType.BezierSeed] = function (
              seed,
              endPoint,
              reversed,
              d,
            ...
            function chainToSVGPathData(chain, offset, accuracy) {
              ...
              var firstPoint = offsetPoint(
                svgCoords(first.endPoints[first.reversed ? 1 : 0])
              );
              var d = [
                'M',
              ...
              for (var i = 0; i < chain.links.length; i++) {
                ...
                if (fn) {
                  ...
                  fn(
                    fixedPath,
                    offsetPoint(
                      svgCoords(link.endPoints[link.reversed ? 0 : 1])
                    ),
                    link.reversed,
                    d,
                    accuracy
            ...
            svgPathDataMap[MakerJs.pathType.Line] = function (line, accuracy) {
              return startSvgPathData(
                line.origin,
                MakerJs.point.rounded(line.end, accuracy),
                accuracy
              );
            };
            svgPathDataMap[MakerJs.pathType.Circle] = function (
              circle,
              accuracy,
              clockwiseCircle
            ) {
              return startSvgPathData(
                circle.origin,
                svgCircleData(circle.radius, accuracy, clockwiseCircle),
                accuracy
              );
            };
            ...
            svgPathDataMap[MakerJs.pathType.BezierSeed] = function (
              seed,
              accuracy
            ) {
              var d = [];
              ...
              MakerJs.model.findChains(
                ...
                function (chains, loose, layer) {
                  function single(walkedPath, clockwise) {
                    var pathData = pathToSVGPathData(
                      walkedPath.pathContext,
                      walkedPath.offset,
                      offset,
                      accuracy,
            ...
            function toSVGPathData(modelToExport) {
              ...
              var pathDataArrayByLayer = getPathDataByLayer(
                modelToExport,
                options.origin,
                findChainsOptions,
                options.accuracy
              ...
              for (var layer in pathDataArrayByLayer) {
                pathDataStringByLayer[layer] =
                  pathDataArrayByLayer[layer].join(' ');
              }
              return findChainsOptions.byLayers
                ? pathDataStringByLayer
                : pathDataStringByLayer[''];
            }
            ...
            function toSVG(itemToExport, options) {
              ...
              function fixPath(pathToFix, origin) {
                ...
                return MakerJs.path.moveRelative(
                  MakerJs.path.scale(mirrorY, opts.scale),
                  origin
                );
              }
              //fixup options
              ...
              if (itemToExportIsModel) {
                ...
                if (modelToExport.exporterOptions) {
                  MakerJs.extendObject(
                    opts,
                    modelToExport.exporterOptions['toSVG']
                  );
                }
              }
              var elements = [];
              ...
              captions.forEach(function (caption) {
                MakerJs.measure.increase(
                  size,
                  MakerJs.measure.pathExtents(caption.anchor),
                  true
                );
              });
              //try to get the unit system from the itemToExport
              ...
              if (size && opts.viewBox) {
                var width = MakerJs.round(
                  size.width * opts.scale,
                  opts.accuracy
                );
                var height = MakerJs.round(
                  size.height * opts.scale,
                  opts.accuracy
                );
              ...
              var svgTag = new exporter.XmlTag(
                'svg',
                MakerJs.extendObject(svgAttrs, opts.svgAttrs)
              );
              ...
              if (opts.useSvgPathOnly) {
                ...
                var pathDataByLayer = getPathDataByLayer(
                  modelToExport,
                  opts.origin,
                  findChainsOptions,
                  opts.accuracy
                ...
                map[MakerJs.pathType.Line] = function (
                  ...
                  if (flow) {
                    addFlowMarks(
                      ...
                      line.origin,
                      line.end,
                      MakerJs.angle.ofLineInDegrees(line)
                    );
                  }
                };
                map[MakerJs.pathType.Circle] = function (
                ...
                map[MakerJs.pathType.Arc] = function (
                  id,
                  arc,
                  layer,
                  className,
                  ...
                  var arcPoints = MakerJs.point.fromArc(arc);
                  if (
                    MakerJs.measure.isPointEqual(arcPoints[0], arcPoints[1])
                  ) {
                    circleInPaths(
                      id,
                      arc.origin,
                    ...
                    if (flow) {
                      addFlowMarks(
                        ...
                        arcPoints[1],
                        arcPoints[0],
                        MakerJs.angle.noRevolutions(arc.startAngle - 90)
                      );
                    }
                  }
                };
                map[MakerJs.pathType.BezierSeed] = function (
                  id,
                  seed,
                  layer,
                  className,
                ...
                function addFlowMarks(flow, layer, origin, end, endAngle) {
                  ...
                  map[MakerJs.pathType.Circle](
                    '',
                    new MakerJs.paths.Circle(origin, flow.size / 2),
                    layer,
                    className,
                    null,
                    false,
                  ...
                  var arrowLines = [
                    arrowEnd,
                    MakerJs.point.mirror(arrowEnd, false, true),
                  ].map(function (p) {
                    return new MakerJs.paths.Line(
                      MakerJs.point.add(MakerJs.point.rotate(p, endAngle), end),
                      end
                    );
                  });
                  arrowLines.forEach(function (a) {
                    return map[MakerJs.pathType.Line](
                      '',
                      a,
                      layer,
                      className,
                ...
                var walkOptions = {
                  ...
                  onPath: function (walkedPath) {
                    var fn = map[walkedPath.pathContext.type];
                    if (fn) {
                      var offset = MakerJs.point.add(
                        fixPoint(walkedPath.offset),
                        opts.origin
                      );
                      fn(
                        walkedPath.pathId,
                        fixPath(walkedPath.pathContext, offset),
                        walkedPath.layer,
                        null,
                        walkedPath.route,
                        opts.annotate,
              ...
              var captionTags = captions.map(function (caption) {
                ...
                var center = MakerJs.point.rounded(
                  MakerJs.point.middle(anchor),
                  opts.accuracy
                );
                var tag = new exporter.XmlTag('text', {
                  ...
                  'text-anchor': 'middle',
                  transform: 'rotate('
                    .concat(MakerJs.angle.ofLineInDegrees(anchor), ',')
                    .concat(center[0], ',')
                    .concat(center[1], ')'),
                  x: center[0],
                  y: center[1],
              ...
              if (captionTags.length) {
                ...
                addSvgAttrs(
                  captionGroup.attrs,
                  colorLayerOptions(captionGroup.attrs.id)
                );
            ...
            function svgBezierData(d, seed, accuracy, reversed) {
              if (seed.controls.length === 1) {
                d.push(
                  'Q',
                  MakerJs.round(seed.controls[0][0], accuracy),
                  MakerJs.round(seed.controls[0][1], accuracy)
                );
              } else {
                var controls = reversed
                  ? [seed.controls[1], seed.controls[0]]
                  : seed.controls;
                d.push(
                  'C',
                  MakerJs.round(controls[0][0], accuracy),
                  MakerJs.round(controls[0][1], accuracy),
                  MakerJs.round(controls[1][0], accuracy),
                  MakerJs.round(controls[1][1], accuracy)
                );
              }
              var final = reversed ? seed.origin : seed.end;
              d.push(
                MakerJs.round(final[0], accuracy),
                MakerJs.round(final[1], accuracy)
              );
            }
            /**
             * @private
            ...
            function svgArcData(
              ...
              d.push(
                MakerJs.round(end[0], accuracy),
                MakerJs.round(end[1], accuracy)
              );
            }
            /**
          ...
          (function (importer) {
            ...
            function fromSVGPathData(pathData, options) {
              ...
              function getPoint(cmd, offset, from) {
                ...
                if (offset < 0) {
                  // get point from end of list (negative index)
                  offset = offset + cmd.data.length;
                }
                var p = MakerJs.point.mirror(
                  [cmd.data[0 + offset], cmd.data[1 + offset]],
                  false,
                  true
                );
                if (cmd.absolute) {
              ...
              map['A'] = function (cmd) {
                ...
                for (var a = 0; a < cmd.data.length; a = a + 7) {
                  ...
                  xAxis = new MakerJs.paths.Line(
                    cmd.from,
                    MakerJs.point.rotate(end, rotation, cmd.from)
                  );
                  ...
                  if (elliptic) {
                    ...
                    e = new MakerJs.models.EllipticArc(
                      arc,
                      1,
                      ry / rx,
                      options.bezierAccuracy
                    );
              ...
              var commandMatches;
              while (
                (commandMatches = regexpCommands.exec(pathData)) !== null
              ) {
        ...
        (function (MakerJs) {
          ...
          (function (layout) {
            ...
            function getChildPlacement(parentModel, baseline) {
              ...
              var measureParent = MakerJs.measure.modelExtents(
                parentModel,
                atlas
              );
              ...
              var walkOptions = {
                beforeChildWalk: function (context) {
                  ...
                  MakerJs.model.originate(child, [
                    childMeasure.center[0],
                    parentTop * baseline,
                  ]);
            ...
            onPathMap[MakerJs.pathType.Arc] = function (arc, reversed, cpa) {
              ...
              cpa.forEach(function (p) {
                return (p.angle = reversed
                  ? arc.endAngle - p.xRatio * arcSpan - 90
                  : arc.startAngle + p.xRatio * arcSpan + 90);
              });
            };
            ...
            function childrenOnPath(
              ...
              cpa.forEach(function (p) {
                return (p.origin = MakerJs.point.middle(
                  chosenPath,
                  reversed ? 1 - p.xRatio : p.xRatio
                ));
              });
            ...
            function miterAngles(points, offsetAngle) {
              ...
              return points.map(function (p, i) {
                ...
                if (i === 0) {
                  ...
                  arc.origin = p;
                  arc.startAngle = MakerJs.angle.ofPointInDegrees(
                    p,
                    points[i + 1]
                  );
                  arc.endAngle = MakerJs.angle.ofPointInDegrees(
                    p,
                    points[i - 1]
                  );
            ...
            function childrenOnChain(
              ...
              if (cpa.length > 1) {
                ...
                points.push(
                  onChain.links[onChain.links.length - 1].endPoints[
                    onChain.links[onChain.links.length - 1].reversed ? 0 : 1
                  ]
                );
              }
            ...
            function cloneToRadial(
              ...
              for (var i = 0; i < count; i++) {
                add[i] = rotateFn(
                  MakerJs.cloneObject(itemToClone),
                  i * angleInDegrees,
                  rotationOrigin
                );
              }
            ...
            function cloneToGrid(itemToClone, xCount, yCount, margin) {
              ...
              return cloneToColumn(
                cloneToRow(itemToClone, xCount, margins[0]),
                yCount,
                margins[1]
              );
            }
            ...
            function cloneToAlternatingRows(
              ...
              for (var i = 0; i < yCount; i++) {
                var i2 = i % 2;
                result.models[i] = MakerJs.model.move(
                  cloneToRow(itemToClone, xCount + i2, spacing.xMargin),
                  [i2 * spacing.x, i * spacing.y]
                );
              }
              return result;
            }
        ...
        (function (MakerJs) {
          ...
          (function (models) {
            ...
            function getExtrema(b) {
              var extrema = b
                .extrema()
                .values//round the numbers so we can compare them to each other
                .map(function (m) {
            ...
            function getError(b, startT, endT, arc, arcReversed) {
              ...
              function m(ratio) {
                ...
                var ap = MakerJs.point.middle(
                  arc,
                  arcReversed ? 1 - ratio : ratio
                );
            ...
            function getLargestArc(b, startT, endT, accuracy) {
              ...
              while (count < 100) {
                ...
                if (reversed === undefined) {
                  reversed = MakerJs.measure.isPointEqual(
                    start.point,
                    MakerJs.point.fromAngleOnCircle(arc.endAngle, arc)
                  );
                }
                ...
                if (
                  ...
                  (lastGoodArc &&
                    lastGoodArc !== arc &&
                    MakerJs.angle.ofArcSpan(arc) -
                      MakerJs.angle.ofArcSpan(lastGoodArc) <
                      0.5)
                ) {
                  return lastGoodArc;
                }
            ...
            function getActualBezierRange(curve, arc, endpoints, offset) {
              ...
              var tPoints = [arc.bezierData.startT, arc.bezierData.endT].map(
                function (t) {
            ...
            function getChainBezierRange(curve, c, layer, addToLayer) {
              ...
              if (
                endLinks[0].walkedPath.pathContext.bezierData.startT >
              ...
              var result = {
                startT: actualBezierRanges[0]
                  ? actualBezierRanges[0].startT
                  : null,
              ...
              if (result.startT !== null && result.endT !== null) {
                ...
                if (result.startT === null) {
                  ...
                  result.startT =
                    c.links[1].walkedPath.pathContext.bezierData.startT;
                }
                if (result.endT === null) {
                  //exclude the last from the chain
                  addToLayer(
                    c.links[c.links.length - 1].walkedPath.pathContext,
                    layer,
                    true
                  );
                  result.endT =
                    c.links[
                      c.links.length - 2
                    ].walkedPath.pathContext.bezierData.endT;
                }
                return result;
              }
              return null;
            ...
            var BezierCurve = /** @class */ (function () {
              function BezierCurve() {
                ...
                if (MakerJs.measure.isBezierSeedLinear(this.seed)) {
                  //use a line and exit
                  var line = new MakerJs.paths.Line(
                    MakerJs.point.clone(this.seed.origin),
                    MakerJs.point.clone(this.seed.end)
                  );
                  line.bezierData = {
                    startT: 0,
                    endT: 1,
              ...
              BezierCurve.getBezierSeeds = function (curve, options) {
                ...
                var addToLayer = function (pathToAdd, layer, clone) {
                  ...
                  seedsByLayer[layer].push(
                    clone ? MakerJs.path.clone(pathToAdd) : pathToAdd
                  );
                };
                MakerJs.model.findChains(
                  curve,
                  function (chains, loose, layer) {
                    ...
                    loose.forEach(function (wp) {
                      ...
                      var range = getActualBezierRange(
                        curve,
                        wp.pathContext,
                        MakerJs.point.fromPathEnds(wp.pathContext),
                        wp.offset
                      );
                      if (range) {
                        var b = getScratch(curve.seed);
        ...
        (function (MakerJs) {
          ...
          (function (models) {
            ...
            function controlPointsForCircularCubic(arc) {
              ...
              var y = controlYForCircularCubic(
                MakerJs.angle.toRadians(arcSpan)
              );
              //multiply by radius
              var c1 = [arc.radius, arc.radius * y];
              //get second control point by mirroring, then rotating
              var c2 = MakerJs.point.rotate(
                MakerJs.point.mirror(c1, false, true),
                arcSpan,
                [0, 0]
              );
              //rotate again to start angle, then offset by arc's origin
              return [c1, c2].map(function (p) {
                return MakerJs.point.add(
                  arc.origin,
                  MakerJs.point.rotate(p, arc.startAngle, [0, 0])
                );
              });
            }
            /**
              ...
              function Ellipse() {
                ...
                var realArgs = function (numArgs) {
                  ...
                  for (var i = 0; i < n; i++) {
                    ...
                    _this.models['Curve_' + (1 + i)] = new models.BezierCurve(
                      seed,
                      accuracy
                    );
                    arc.startAngle += a;
            ...
            var EllipticArc = /** @class */ (function () {
              function EllipticArc() {
                ...
                for (var i = 0; i < count; i++) {
                  ...
                  this.models['Curve_' + (1 + i)] = new models.BezierCurve(
                    seed,
                    accuracy
                  );
                }
            ...
            var ConnectTheDots = /** @class */ (function () {
              function ConnectTheDots() {
                ...
                var connect = function (a, b, skipZeroDistance) {
                  ...
                  if (
                    skipZeroDistance &&
                    MakerJs.measure.pointDistance(points[a], points[b]) == 0
                  )
                    return;
                  _this.paths['ShapeLine' + i] = new MakerJs.paths.Line(
                    points[a],
                    points[b]
                  );
                };
          ...
          (function (models) {
            var Polygon = /** @class */ (function () {
              function Polygon(
                ...
                this.paths = {};
                this.paths = new models.ConnectTheDots(
                  true,
                  Polygon.getPoints(
                    numberOfSides,
                    radius,
            ...
            var Holes = /** @class */ (function () {
              ...
              function Holes(holeRadius, points, ids) {
                ...
                for (var i = 0; i < points.length; i++) {
                  ...
                  this.paths[id] = new MakerJs.paths.Circle(
                    points[i],
                    holeRadius
                  );
                }
          ...
          (function (models) {
            var BoltCircle = /** @class */ (function () {
              function BoltCircle(
                ...
                holeRadius,
                boltCount,
                firstBoltAngleInDegrees
              ) {
                if (firstBoltAngleInDegrees === void 0) {
                  firstBoltAngleInDegrees = 0;
                }
                this.paths = {};
                var points = models.Polygon.getPoints(
                  boltCount,
                  boltRadius,
                  firstBoltAngleInDegrees
                );
          ...
          (function (models) {
            var Dogbone = /** @class */ (function () {
              ...
              function Dogbone(width, height, radius, style, bottomless) {
                ...
                if (bottomless) {
                  this.paths['Left'] = new MakerJs.paths.Line(
                    [0, 0],
                    [0, height - ly]
                  );
                  this.paths['Right'] = new MakerJs.paths.Line(
                    [width, 0],
                    [width, height - ly]
                  );
                } else {
                  ...
                  this.paths['Bottom'] = new MakerJs.paths.Line(
                    [lx, 0],
                    [width - lx, 0]
                  );
                  this.paths['BottomLeft'] = new MakerJs.paths.Arc(
                    [ax, ay],
                    radius,
                    apexes[0] - 90,
                    apexes[0] + 90
                  );
                  this.paths['BottomRight'] = new MakerJs.paths.Arc(
                    [width - ax, ay],
                    radius,
                    apexes[1] - 90,
                    apexes[1] + 90
                ...
                this.paths['TopRight'] = new MakerJs.paths.Arc(
                  [width - ax, height - ay],
                  radius,
                  apexes[2] - 90,
                  apexes[2] + 90
                );
                this.paths['TopLeft'] = new MakerJs.paths.Arc(
                  [ax, height - ay],
                  radius,
                  apexes[3] - 90,
                  apexes[3] + 90
                );
                this.paths['Top'] = new MakerJs.paths.Line(
                  [lx, height],
                  [width - lx, height]
                );
              }
              ...
              function RoundRectangle() {
                ...
                switch (args.length) {
                  ...
                  case 1:
                    ...
                    this.origin = MakerJs.point.subtract(m.low, [
                      radius,
                      radius,
                    ]);
          ...
          (function (models) {
            var Oval = /** @class */ (function () {
              function Oval(width, height) {
                this.paths = {};
                this.paths = new models.RoundRectangle(
                  width,
                  height,
            ...
            var OvalArc = /** @class */ (function () {
              function OvalArc(
                ...
                var addCap = function (
                  ...
                  return (capModel.paths[id] = new MakerJs.paths.Arc(
                    MakerJs.point.fromPolar(
                      MakerJs.angle.toRadians(tiltAngle),
                      sweepRadius
                    ),
                    slotRadius,
                ...
                var addSweep = function (id, offsetRadius) {
                  return (_this.paths[id] = new MakerJs.paths.Arc(
                    [0, 0],
                ...
                if (d / 2 < slotRadius) {
                  ...
                  if (int) {
                    ...
                    if (
                      !selfIntersect &&
                      hasInner &&
                      int.intersectionPoints.length == 2
                    ) {
                      addCap('StartCap2', startAngle, 180, 0).endAngle =
                        int.path1Angles[1];
                      addCap('EndCap2', endAngle, 0, 180).startAngle =
                        int.path2Angles[1] + 360;
                    }
                  }
                }
              }
          ...
          (function (models) {
            var Belt = /** @class */ (function () {
              function Belt(leftRadius, distance, rightRadius) {
                ...
                var right = new MakerJs.paths.Arc(
                  [distance, 0],
                  rightRadius,
                  0,
                  360
                ...
                if (!angles) {
                  this.paths['Belt'] = new MakerJs.paths.Circle(
                  ...
                  this.paths['Top'] = new MakerJs.paths.Line(
                    MakerJs.point.fromAngleOnCircle(angles[0], left),
                    MakerJs.point.fromAngleOnCircle(angles[0], right)
                  );
                  this.paths['Bottom'] = new MakerJs.paths.Line(
                    MakerJs.point.fromAngleOnCircle(angles[1], left),
                    MakerJs.point.fromAngleOnCircle(angles[1], right)
                  );
                }
              }
              return Belt;
            ...
            var SCurve = /** @class */ (function () {
              function SCurve(width, height) {
                ...
                if (width > height) {
                  ...
                  startAngle = 270;
                  endAngle =
                    360 - MakerJs.angle.toDegrees(Math.acos(w2 / radius));
                  arcOrigin = [0, radius];
                } else {
                  radius = findRadius(w2, h2);
                  startAngle =
                    180 - MakerJs.angle.toDegrees(Math.asin(h2 / radius));
                  endAngle = 180;
                  arcOrigin = [radius, 0];
                }
                var curve = new MakerJs.paths.Arc(
                  arcOrigin,
                  radius,
                  startAngle,
                  endAngle
                ...
                this.paths['curve_end'] = MakerJs.path.moveRelative(
                  MakerJs.path.mirror(curve, true, true),
                  [width, height]
                );
              }
              return SCurve;
          ...
          (function (models) {
            var Slot = /** @class */ (function () {
              function Slot(origin, endPoint, radius, isolateCaps) {
                ...
                addCap(
                  'StartCap',
                  new MakerJs.paths.Arc([0, 0], radius, 90, 270)
                );
                addCap(
                  'EndCap',
                  new MakerJs.paths.Arc([len, 0], radius, 270, 90)
                );
            ...
            var Star = /** @class */ (function () {
              function Star(
                ...
                if (!innerRadius) {
                  innerRadius =
                    outerRadius *
                    Star.InnerRadiusRatio(numberOfPoints, skipPoints);
                }
                var outerPoints = models.Polygon.getPoints(
                  numberOfPoints,
                  outerRadius
                );
                var innerPoints = models.Polygon.getPoints(
                  numberOfPoints,
                  innerRadius,
                  180 / numberOfPoints
                );
              ...
              Star.InnerRadiusRatio = function (numberOfPoints, skipPoints) {
                ...
                if (
                  numberOfPoints > 0 &&
                  skipPoints > 1 &&
                  skipPoints < numberOfPoints / 2
                ) {
                  return (
                    Math.cos((Math.PI * skipPoints) / numberOfPoints) /
                    Math.cos((Math.PI * (skipPoints - 1)) / numberOfPoints)
                  );
                }
                return 0;
              };
            ...
            var Text = /** @class */ (function () {
              ...
              function Text(
                ...
                var cb = function (glyph, x, y, _fontSize, options) {
                  var charModel = Text.glyphToModel(
                    glyph,
                    _fontSize,
                    bezierAccuracy
                  );
                  ...
                  if (
                    centerCharacterOrigin &&
                    (charModel.paths || charModel.models)
                  ) {
